...
深度学习的兴起为解决复杂优化问题提供了新的思路。近年来，机器学习与通信理论的交叉研究取得了显著进展。文献\upcite{3}率先将全连接网络（FCN）应用于MIMO预编码设计，证明了数据驱动方法的可行性；文献\upcite{4}利用图神经网络（GNN）建模用户间干扰关系，实现了可扩展的功率分配方案；文献\upcite{5}提出了基于强化学习的动态资源分配框架，适用于时变信道环境。然而，现有深度学习方法普遍存在以下局限：
...
\hline
文献\upcite{3} & 实虚分离 & 监督学习 & 约5\% & $O(N_t^2 K)$ \\
文献\upcite{4} & 实虚分离 & 监督学习 & 约3\% & $O(N_t K^2)$ \\
文献\upcite{11} & 复数运算 & 强化学习 & 约8\% & $O(N_t^3)$ \\
